...
class NatsUtil {
  ...
  constructor(clusterId?: string, servers?: string[], token?: string) {
  ...
  getNatsConnectionOpts(clientId: string): nats.ClientOpts {
    return {
  ...
  async getNatsConnection(): Promise<NATSClient> {
    ...
    try {
      ...
      client = await nats.connect(opts) as NATSClient;
  ...
  async getNatsStreamingConnection(): Promise<NATSStreamingClient> {
    ...
    try {
      ...
      client = await stan.connect(this.clusterId, clientId, opts) as NATSStreamingClient;
    } catch (err) {
